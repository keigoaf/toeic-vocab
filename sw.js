/* ====================================================================
   ...
   最終更新日時: 2026-07-29 (JST)  /  対応アプリ版: v3.9.0
   --------------------------------------------------------------------
   【役割】
   ...
   【v1.8.0 での変更理由】
     ...
     引っぱって更新もできず、再起動でも直らないため、以下を実施:
       ...
       4. ページ側からの 'SKIP_WAITING' メッセージに対応し、
          更新検知時に即時反映できるようにする。
   【v3.7.0 での変更理由】
     単語Tips機能の追加に伴い word-relations.json を差し替えたため、
     キャッシュ名を v3.7.0 に更新して旧キャッシュを確実に破棄する。
     （JSONはネットワーク優先だが、オフライン時の取り置き分も入れ替える）

   【v3.8.0 での変更理由】
     シャドーイング機能を別ページとして追加したため、
     shadowing.html / shadowing.json を事前キャッシュ対象に加えた。
     ※ HTMLはネットワーク優先なので、オンライン時は常に最新が出る。
       ここに入れるのはオフラインでも開けるようにするため。

   【v3.9.0 での変更理由】
     Part5の成績集計・学習記録の追加で index.html を更新したため、
     キャッシュ名を v3.9.0 に更新して旧キャッシュを確実に破棄する。

   【更新手順】
     ファイルを更新したら必ず下の CACHE のバージョンを上げること。
     activate で旧キャッシュが削除され、確実に入れ替わる。
   ==================================================================== */
const CACHE = 'toeic-vocab-v3.9.0';
